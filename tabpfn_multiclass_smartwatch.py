...
from tabpfn_multiclass import load_data, build_pipeline, run_variant
from logreg_multiclass_smartwatch import EDA_COLS, TEMP_COLS
from xgboost_multiclass import ID_COLS, LEAK_COLS, DESIGN_COLS, EMPTY_COLS, ACC_COLS, GROUP, SCORING, SCORING_LABELS
...
def build_features_smartwatch(df, exclude_acc=False):
    drop = set(ID_COLS + LEAK_COLS + DESIGN_COLS + EMPTY_COLS + EDA_COLS + TEMP_COLS)
    if exclude_acc:
        drop |= set(ACC_COLS)
...
def run_variant_generic(X, y, groups, feature_cols, le, name):
    """Manueller LOSOCV-Loop, identisch zu tabpfn_multiclass.run_variant,
    aber entkoppelt von build_features(df, include_acc) - nimmt fertige
    X/y/groups/feature_cols entgegen."""
    cv = LeaveOneGroupOut()
    n = len(y)
    y_pred = np.empty(n, dtype=int)
    y_prob = np.empty((n, 4), dtype=float)
    ...
    for train_idx, test_idx in cv.split(X, y, groups=groups):
        ...
        y_prob[test_idx] = pipe.predict_proba(X_te)
    ...
    return {
        "name": name, "X": X, "y": y, "groups": groups,
...
def make_figure(variants, out_path):
    ...
    fig.suptitle("TabPFN – 4-Klassen (bereinigt): Smartwatch-Feature-Sets im Vergleich\n"
                 "alle Features / ohne ACC (voller Datensatz) vs. HR+ACC+HRV / nur HR+HRV (Smartwatch)",
    ...
    for label, v_ref in [("alle Features", v_all), ("ohne ACC", v_noacc), ("nur HR+ACC+HRV", v_sw)]:
...
def main():
    ...
    print("TabPFN LOSOCV (CPU), 4 Varianten x 31 Folds - bitte etwas Geduld ...\n")

    v_all = run_variant(df, include_acc=True); v_all["name"] = "alle Features"
    print("... alle Features fertig")
    v_noacc = run_variant(df, include_acc=False); v_noacc["name"] = "ohne ACC"
    print("... ohne ACC fertig")

    X, y, groups, feature_cols, le = build_features_smartwatch(df, exclude_acc=False)
    v_sw = run_variant_generic(X, y, groups, feature_cols, le, "nur HR+ACC+HRV")
    print("... nur HR+ACC+HRV fertig")

    X, y, groups, feature_cols, le = build_features_smartwatch(df, exclude_acc=True)
    v_swnoacc = run_variant_generic(X, y, groups, feature_cols, le, "nur HR+HRV")
    print("... nur HR+HRV fertig")
    ...
    make_figure(variants, Path(__file__).parent / "tabpfn_multiclass_smartwatch_results.png")
